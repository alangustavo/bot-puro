...
class BotEMAConfig extends BotConfig {
    ...
    constructor(
        ...
        public longInterval: Interval = '1h',
        ...
        public trailingStopLossPercent: number = 0.015, // 1.5% Trailing Stop Loss
...
class BotRSI extends Bot {
    ...
    protected async analyzeMarket(): Promise<void> {
        ...
        const rsiLongDayIndicator = i.calculateRSI(this.config.symbol, this.config.longInterval, this.config.rsiLongInterval);
        const rsiShortDayIndicator = i.calculateRSI(this.config.symbol, this.config.longInterval, this.config.rsiShortInterval);
        ...
        const ema23 = i.calculateEMA(this.config.symbol, this.config.interval, 23);
        const ema19 = i.calculateEMA(this.config.symbol, this.config.interval, 19);
        ...
        if (this.botRSIStatus === 'CROSS_UP' && actualEma19 > actualEma23 && actualShortRSI > previoShortRSI && actualLongDayRSI < 70 && actualShortDayRSI < 70) {
        ...
        if (this.status == 'SOLD') {
            ...
            if (this.botIntention == 'BUY'
                ...
                // actualPrice > actualfast50EMA &&
                // upDayRSI &&
                // actualShortDayRSI < 60 &&
                // previoShortDayRSI < actualLongDayRSI
            ) {
                op = 'BUY';
                ...
                RSI S/L.....: ${actualShortRSI.toFixed(3)}/${actualLongdRSI.toFixed(3)}
                STATUS......: ${this.status}`;
                this.log(message);
                opReason += `\nRSI DAY S/L: ${actualShortDayRSI.toFixed(3)}/${actualLongDayRSI.toFixed(3)} ${actualShortDayRSI > actualLongDayRSI ? 'UP' : 'DOWN'}\nPrevious ${previoShortDayRSI.toFixed(3)}/${previoLongDayRSI.toFixed(3)}`;
                opReason += `\nEMA 19/23: ${actualEma19.toFixed(3)}/${actualEma23.toFixed(3)} ${actualEma19 > actualEma23 ? 'UP' : 'DOWN'}`;
                opReason += `\nSTOP LOSS:${this.stopLossPrice}\nSTOP GAIN:${this.stopGainPrice}`;
                ...
                this.stopLossPrice = actualPrice * (1 - stopLoss);
                this.stopGainPrice = actualPrice * (1 + this.config.stopGainPercent);
                console.log(`Stop Loss Price: ${this.stopLossPrice.toFixed(2)}, Stop Gain Price: ${this.stopGainPrice.toFixed(2)}`);
                message = operation.getBuyMessage(actualPrice);
            ...
            if (this.buyPrice !== null) {
                ...
                // this.stopLossPrice = this.buyPrice * (1 - this.config.stopLossPercent);
                // this.stopGainPrice = this.buyPrice * (1 + this.config.stopGainPercent);
                ...
                if (pl > this.traillingStopActual && !this.trailingStopLossActive) {
                    ...
                    console.log(`Active Trailing Stop Loss at ${this.trailingStopLossPrice.toFixed(2)} ${(this.trailingStopLossPrice * 100).toFixed(3)}%`);
                }
            ...
            if (this.botIntention === 'SELL' && actualShortRSI > 70 && minimumCriteria && pl > 0.5) {
                op = 'SELL';
                const message = `
SELL SIGNAL.: ${this.config.symbol} 
BUY PRICE...: ${this.buyPrice ? this.buyPrice.toFixed(2) : 'N/A'}
STOP PRICE..: ${this.stopLossPrice ? this.stopLossPrice.toFixed(2) : 'N/A'}
STOP GAIN...: ${this.stopGainPrice ? this.stopGainPrice.toFixed(2) : 'N/A'}
PRICE ......: ${actualPrice.toFixed(2)} 
...
`;
                ...
                this.sendMessage(message);
            } else if (actualPrice < this.stopLossPrice) {
                console.log(lastKline?.toString());
                op = 'SELL';
                this.trailingStopLossActive = false;
                this.closeOperation(this.stopLossPrice, `STOP LOSS ${opReason}`, actualDate);
...
`;
                this.sendMessage(message);
            } else if (this.stopGainPrice && actualPrice > this.stopGainPrice) {
                op = 'SELL';
                this.trailingStopLossActive = false;
                console.log(lastKline?.toString());
...
`;
                ...
                op = 'SELL';
                this.trailingStopLossActive = false;
                this.log(`Trailing Stop Loss triggered at ${actualPrice.toFixed(2)} with price ${this.trailingStopLossPrice.toFixed(2)}`);
                console.log(lastKline?.toString());
                this.closeOperation(actualPrice, `TRAILING STOP LOSS ${opReason}`, actualDate);
                const message = `SELL SIGNAL.: ${this.config.symbol}
BUY PRICE...: ${this.buyPrice ? this.buyPrice.toFixed(2) : 'N/A'}
PRICE ......: ${actualPrice.toFixed(2)}
STOP PRICE..: ${this.stopLossPrice ? this.stopLossPrice.toFixed(2) : 'N/A'}
